        ...
        const autoSearchScript = `
            <script>
                ...
                (function() {
                    ...
                    // 直接触发搜索 - 不使用模拟点击
                    function triggerSearch(element, searchForm) {
                        try {
                            // 直接启用按钮并提交
                            ...
                            // 直接触发表单提交（如果存在表单）
                            const form = element.closest('form') || searchForm;
                            if (form) {
                                // 直接调用表单的submit方法
                                ...
                                // 如果没有表单，直接触发按钮的onclick事件（如果存在）
                                if (element.onclick && typeof element.onclick === 'function') {
                                    element.onclick();
                                } else {
                                    // 最后的fallback：简单的click
                                    element.click();
                                }
                            }
                            
                        } catch (e) {
                            // 如果所有方法都失败，则使用原生click作为最后的fallback
                            try {
                                element.click();
                            } catch (clickError) {
                                console.error('❌ [AutoSearch] 搜索触发失败:', clickError);
                            }
                        }
                    }
                    ...
                    async function setupScope(scope) {
                        try {
                            ...
                            if (targetTab) {
                                console.log('🔧 [SearchParams] 设置目标元素状态');
                                
                                try {
                                    // 直接修改元素状态而不是模拟点击
                                    if (targetTab.offsetParent !== null) {
                                        // 直接设置按钮为激活状态
                                        targetTab.classList.add('active', 'selected');
                                        targetTab.setAttribute('aria-selected', 'true');
                                        targetTab.setAttribute('data-selected', 'true');
                                        
                                        // 如果是tab角色，更新其状态
                                        if (targetTab.getAttribute('role') === 'tab') {
                                            // 先移除其他tab的选中状态
                    ...
                    async function setupKits(kits) {
                        try {
                            ...
                            if (kitsElement) {
                                ...
                                // 更新元素的数据属性
                                kitsElement.setAttribute('data-value', finalKits);
                                kitsElement.setAttribute('data-kits', finalKits);
                    ...
                    async function executeAutoSearch() {
                        try {
                            ...
                            console.log('🖱️ [AutoSearch] 开始触发搜索...');
                            triggerSearch(sendButton);
