...
app.use('/', proxy('https://metaso.cn', {
    ...
    proxyReqOptDecorator: function(proxyReqOpts, srcReq) {
        ...
        // 添加隐藏左侧菜单的cookie
        const existingCookies = proxyReqOpts.headers['Cookie'] || '';
        const hideLeftMenuCookie = 'hideLeftMenu=1';
        if (existingCookies) {
            proxyReqOpts.headers['Cookie'] = existingCookies + '; ' + hideLeftMenuCookie;
        } else {
            proxyReqOpts.headers['Cookie'] = hideLeftMenuCookie;
        }
        
        console.log(`\n=== 代理请求 ${srcReq.path} ===`);
        console.log('目标URL:', proxyReqOpts.href);
        console.log('设置Cookie:', proxyReqOpts.headers['Cookie']);
...
// 调用隐藏左侧菜单API的函数
function callHideLeftMenuAPI() {
    const options = {
        hostname: 'metaso.cn',
        port: 443,
        path: '/api/presentation/config',
        method: 'GET',
        headers: {
    ...
    const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
            try {
                ...
                console.log('隐藏左侧菜单API调用成功:', result);
            } catch (e) {
                console.log('隐藏左侧菜单API响应:', data);
            }
        });
    });

    req.on('error', (e) => {
        console.error('隐藏左侧菜单API调用失败:', e.message);
    });
