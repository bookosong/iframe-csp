...
function buildProxyServerUrl() {
    const protocol = 'http:'; // 本地开发环境使用HTTP
...
app.get('/*.html', (req, res, next) => {
    ...
    if (fs.existsSync(filePath)) {
        ...
        fs.readFile(filePath, 'utf8', (err, html) => {
            if (err) {
                logger.error(`Failed to read HTML file: ${fileName}`, err);
                return res.status(500).send('Internal Server Error');
            }
            
            try {
                // 对本地HTML文件也应用URL替换处理
                const processedHtml = processHtmlResponse(html, req.path);
                res.setHeader('Content-Type', 'text/html; charset=utf-8');
                res.send(processedHtml);
            } catch (processError) {
                logger.error(`Failed to process HTML file: ${fileName}`, processError);
                // 如果处理失败，返回原始内容
                res.setHeader('Content-Type', 'text/html; charset=utf-8');
                res.send(html);
            }
            ...
            <script>
                ...
                function interceptImageLoading() {
                    try {
                        ...
                        console.error(logPrefix, '图片URL替换失败:', e);
                    }
                }
