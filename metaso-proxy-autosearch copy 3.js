                    ...
                    async function executeAutoSearch() {
                        try {
                            ...
                            try {
                                ...
                                console.warn('⚠️ [AutoSearch] 直接API调用失败，尝试其他方法:', apiError);
                                
                                try {
                                    // 方法2: 通过表单提交方式
                                    console.log('🔧 [AutoSearch] 尝试表单提交方式...');
                                    ...
                                    console.log('✅ [AutoSearch] 表单提交执行完成');
                                    
                                } catch (formError) {
                                    console.warn('⚠️ [AutoSearch] 表单提交失败，使用备用方法:', formError);
                                    
                                    // 方法3: 备用 - 修改当前页面状态和URL
                                    console.log('🔧 [AutoSearch] 使用备用方法 - 状态修改...');
                                    ...
                                    // 触发搜索状态变更事件
                                    const searchEvent = new CustomEvent('autoSearchExecuted', {
                                        detail: { query: finalQuery, method: 'direct-assignment' }
                                    });
                                    window.dispatchEvent(searchEvent);
                                    document.dispatchEvent(searchEvent);
                                    
                                    console.log('✅ [AutoSearch] 备用方法执行完成');
                                }
                            }
