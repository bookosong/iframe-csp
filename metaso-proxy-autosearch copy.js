...
function generateUID() {
    ...
    return '68775c6659a307e8ac864bf6';//timestamp + randomBytes;
}
...
const GLOBAL_SID = 'e5874318e9ee41788605c88fbe43ab19';//generateSID();
        ...
        const scopeSetupScript = `
            <script>
                ...
                (function() {
                    ...
                    console.log('🔧 [ScopeSetup] 检测到参数:', { scope, kits, path: window.location.pathname });
                    
                    // === 新增：保存目标参数供客户端自动搜索脚本使用 ===
                    if (scope) {
                    ...
                    if (kits) {
                        ...
                        console.log('🎯 [ScopeSetup] 已保存目标kits参数:', kits);
                    }
                ...
                (function() {
                    ...
                    async function setupScope(scope) {
                        try {
                            ...
                            // 等待页面元素加载 - 增加等待时间确保所有元素都加载完成
                            console.log('⏳ [SearchParams] 等待页面元素加载完成...');
                            await new Promise(resolve => setTimeout(resolve, 5000));
                            
                            // 等待React组件完全渲染 - 主动检测关键元素是否存在
                            console.log('⏳ [SearchParams] 等待React scope组件渲染...');
                            let retryCount = 0;
                            const maxRetries = 20;
                            ...
                            while (retryCount < maxRetries) {
                                ...
                                console.log('⏳ [SearchParams] 检测到 ' + foundElements + ' 个scope相关元素 (尝试 ' + (retryCount + 1) + '/' + maxRetries + ')');
                                
                                if (foundElements > 0) {
                                    console.log('✅ [SearchParams] React scope组件已渲染完成');
                                    break;
                                }
                            ...
                            if (retryCount >= maxRetries) {
                                console.warn('⚠️ [SearchParams] 等待scope元素超时，继续执行...');
                            }
                                    ...
                                    console.log('[SearchParams] 找到 ' + tabs.length + ' 个域标签 (' + selector + ')');
                                        ...
                                        for (const tab of tabs) {
                                            ...
                                            console.log('    - 编码信息:', {
                                                tabText: tabText,
                                                scope: scope,
                                                tabEncoded: tabText ? encodeURIComponent(tabText) : '',
                                                scopeEncoded: encodeURIComponent(scope),
                                                exactMatch: tabText === scope,
                                                normalizedMatch: tabText && tabText.normalize() === scope.normalize(),
                                                encodedMatch: tabText && encodeURIComponent(tabText) === encodeURIComponent(scope)
                                            });
                                            
                                            // 使用更严格的字符串比较，处理编码问题
                                            if (tabText && (tabText === scope || 
                                                          tabText.normalize() === scope.normalize() ||
                                                          encodeURIComponent(tabText) === encodeURIComponent(scope))) {
                                                targetTab = tab;
                                    ...
                                    try {
                                        ...
                                        for (const subject of subjects) {
                                            ...
                                            console.log('    - 编码信息:', {
                                                ...
                                                scope: scope,
                                                subjectEncoded: subjectText ? encodeURIComponent(subjectText) : '',
                                                scopeEncoded: encodeURIComponent(scope),
                                                exactMatch: subjectText === scope,
                                                normalizedMatch: subjectText && subjectText.normalize() === scope.normalize(),
                                                encodedMatch: subjectText && encodeURIComponent(subjectText) === encodeURIComponent(scope)
                                            });
                                            
                                            // 使用更严格的字符串比较，处理编码问题
                                            if (subjectText && (subjectText === scope || 
                                                              subjectText.normalize() === scope.normalize() ||
                                                              encodeURIComponent(subjectText) === encodeURIComponent(scope))) {
                                                targetTab = subject;
                            ...
                            // 如果仍然没找到，尝试通用文本搜索
                            if (!targetTab) {
                                console.log('[SearchParams] 专题标签未找到，尝试通用文本搜索...');
                                
                                // 搜索所有包含目标文本的可点击元素
                                const allClickableElements = document.querySelectorAll('button, div[role="button"], a, span[role="button"], li[role="menuitem"], [onclick], .MuiTypography-root, div[class*="css-"], span[class*="css-"], div, span, label');
                                console.log('[SearchParams] 找到 ' + allClickableElements.length + ' 个可点击元素');
                                ...
                                for (const element of allClickableElements) {
                                    ...
                                    const combinedText = (elementText + ' ' + elementTitle).toLowerCase();
                                    const scopeLower = scope.toLowerCase();
                                    
                                    // 增强的匹配逻辑
                                    const isMatch = elementText && (
                                        elementText === scope ||
                                        elementText.includes(scope) ||
                                        elementTitle.includes(scope) ||
                                        combinedText.includes(scopeLower) ||
                                        // 针对"泥沙知识"的特殊匹配
                                        (scope === '泥沙知识' && (
                                            combinedText.includes('泥沙') || 
                                            combinedText.includes('知识') ||
                                            elementText.includes('泥沙') ||
                                            elementText.includes('知识')
                                        )) ||
                                        // 部分匹配策略
                                        (scope.length > 2 && elementText.includes(scope.substring(0, 2))) ||
                                        // 去除空格匹配
                                        elementText.replace(/\s+/g, '') === scope.replace(/\s+/g, '') ||
                                        // Unicode标准化匹配
                                        (elementText.normalize('NFD') === scope.normalize('NFD'))
                                    );
                                    
                                    if (isMatch) {
                                        console.log('🔍 [SearchParams] 通用搜索找到匹配元素:', {
                                            text: elementText,
                                            title: elementTitle,
                                            tagName: element.tagName,
                                            className: element.className,
                                            matchReason: elementText === scope ? 'exact' : 
                                                        elementText.includes(scope) ? 'contains' :
                                                        combinedText.includes(scopeLower) ? 'combined' : 'special'
                                        });
                                        
                                        // 优先选择可见且可点击的元素
                                        if (element.offsetParent !== null && !element.disabled && element.style.visibility !== 'hidden') {
                                            targetTab = element;
                                            console.log('🎯 [SearchParams] 选择可见的匹配元素');
                                            break;
                                        } else if (!targetTab) {
                                            // 如果没有找到更好的，至少记录这个元素
                                            targetTab = element;
                                            console.log('📝 [SearchParams] 暂时记录匹配元素（可能不可见）');
                                        }
                                    }
                                }
                            }
                            ...
                            if (targetTab) {
                                console.log('🔧 [SearchParams] 准备点击目标元素:', {
                                    tagName: targetTab.tagName,
                                    className: targetTab.className,
                                    text: targetTab.textContent && targetTab.textContent.trim(),
                                    visible: targetTab.offsetParent !== null,
                                ...
                                try {
                                    ...
                                    // 确保元素可见且可点击
                                    if (targetTab.offsetParent !== null || targetTab.style.display !== 'none') {
                                        // 尝试多种点击方式
                                        console.log('🖱️ [SearchParams] 尝试标准点击...');
                                        targetTab.click();
                                        
                                        // 如果标准点击失败，尝试模拟鼠标事件
                                        await new Promise(resolve => setTimeout(resolve, 200));
                                        
                                        console.log('🖱️ [SearchParams] 尝试鼠标事件...');
                                        ...
                                        for (const selector of ariaLabelSelectors) {
                                            try {
                                                ...
                                                for (const button of ariaButtons) {
                                                    ...
                                                    if (oldLabel && (oldLabel.includes('范围:') || oldLabel.includes('范围：'))) {
                                                        const newLabel = oldLabel.includes('范围:') ? 
                                                            oldLabel.replace(/范围:[^,]*/, '范围:' + scope) :
                                                            oldLabel.replace(/范围：[^,]*/, '范围：' + scope);
                                        ...
                                        // === 新增：验证设置是否成功 ===
                                        await new Promise(resolve => setTimeout(resolve, 1000));
                                        console.log('🔍 [SearchParams] 验证scope设置结果...');
                                        
                                        // 检查按钮状态
                                        const activeButtons = document.querySelectorAll('button[aria-selected="true"], button.active, button[data-selected="true"]');
                                        for (const button of activeButtons) {
                                            const buttonText = button.textContent && button.textContent.trim();
                                            if (buttonText === scope) {
                                                console.log('✅ [SearchParams] 验证成功: scope已设置为 "' + scope + '"');
                                                return;
                                            }
                                        }
                                        
                                        console.log('⚠️ [SearchParams] 验证警告: 无法确认scope设置是否成功');
                                        
                                    } else {
                                        console.warn('⚠️ [SearchParams] 目标元素不可见');
                                        
                                        // 即使不可见也尝试点击
                                        console.log('🖱️ [SearchParams] 尝试点击不可见元素...');
                                        targetTab.click();
                                    ...
                                    // 最后的fallback尝试
                                    try {
                                        console.log('🔄 [SearchParams] 尝试fallback点击方式...');
                                        if (targetTab.click) {
                                            targetTab.click();
                                        }
                                        
                                        // 尝试直接设置value或触发change事件
                                        if (targetTab.tagName === 'INPUT' || targetTab.tagName === 'SELECT') {
                                            targetTab.value = scope;
                                            targetTab.dispatchEvent(new Event('change', { bubbles: true }));
                                        }
                                    } catch (fallbackError) {
                                        console.error('❌ [SearchParams] Fallback点击也失败:', fallbackError.message);
                                    }
                                }
                            } else {
                                ...
                                const allTabs = document.querySelectorAll('button[role="tab"], .subject_tag__Y3JPF div, .MuiBox-root.css-1325aup, button, div[role="button"], span[role="button"]');
                                ...
                                const uniqueOptions = availableOptions
                                    .filter((option, index, self) => 
                                        index === self.findIndex(t => t.text === option.text)
                                    )
                                    .slice(0, 20);
                                    
                                for (const option of uniqueOptions) {
                                    console.log('  - "' + option.text + '" (' + option.tagName + '.' + option.className + ')');
                                }
                                
                                console.log('📝 [SearchParams] 查找到 ' + uniqueOptions.length + ' 个可用选项，目标: "' + scope + '"');
                            }
                    ...
                    async function executeAutoSearch() {
                        try {
                            ...
                            // 执行搜索操作 - 使用动态搜索内容
                            console.log('📝 [AutoSearch] 开始输入搜索内容:', finalQuery);
...
app.use('/', proxy('https://metaso.cn', {
    ...
    proxyReqOptDecorator: function(proxyReqOpts, srcReq) {
        ...
        if (false/*srcReq.path === '/api/searchV2' && srcReq.method === 'POST'*/) {
